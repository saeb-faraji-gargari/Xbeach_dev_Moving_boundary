...
contains
   ...
   subroutine ncoutput_init(s, sl, par, tpar)
      ...
      !NF90(nf90_put_att(ncid, nf90_global, "Producer", &
      !"XBeach littoral zone wave model (" // &
      !"http://www.xbeach.org)"))
   ...
   subroutine ncoutput(s,sl,par, tpar)
      ...
      real*8, dimension(par%nship)                   :: shipFxlocal,shipFylocal,shipFzlocal,shipMxlocal,shipMylocal,shipMzlocal
#endif
      integer                                       :: pii
      ...
      if (dooutput_global) then
         ...
         if (par%nship>0) then
            if(xmaster) then
               shipFxlocal = sl%shipFx
               shipFylocal = sl%shipFy
               shipFzlocal = sl%shipFz
               shipMxlocal = sl%shipMx
               shipMylocal = sl%shipMy
               shipMzlocal = sl%shipMz
            endif
            call xmpi_send(xmpi_imaster,xmpi_omaster,shipFxlocal)
            call xmpi_send(xmpi_imaster,xmpi_omaster,shipFylocal)
            call xmpi_send(xmpi_imaster,xmpi_omaster,shipFzlocal)
            call xmpi_send(xmpi_imaster,xmpi_omaster,shipMxlocal)
            call xmpi_send(xmpi_imaster,xmpi_omaster,shipMylocal)
            call xmpi_send(xmpi_imaster,xmpi_omaster,shipMzlocal)
            if (xomaster) then
               s%shipFx = shipFxlocal
               s%shipFy = shipFylocal
               s%shipFz = shipFzlocal
               s%shipMx = shipMxlocal
               s%shipMy = shipMylocal
               s%shipMz = shipMzlocal
            endif
         endif
...
#ifdef USEMPI
         call hotstartfiles_collect(s,sl,par, tpar)
#endif
      endif
