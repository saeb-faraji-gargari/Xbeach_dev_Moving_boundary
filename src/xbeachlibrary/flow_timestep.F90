...
contains
   subroutine flow(s,par)
      ...
      use spaceparamsdef
      use xmpi_module
      use boundaryconditions,      only: flow_lat_bc, discharge_boundary_h, discharge_boundary_v, water_level_boundary      !***** ----> Saeb
      ...
      real*8                                   :: tolerance=2.d0      !***** ----> Saeb
      real*8                                   :: tolerance_old     !***** ----> Saeb
      ...
      !real*8,dimension(:,:),allocatable,save  ::       zs_previous_time_step   !***** ----> Saeb

      real*8,dimension(:,:),allocatable  ::       zs_old_implicit   !***** ----> Saeb
      real*8,dimension(:,:),allocatable  ::       uu_old_implicit   !***** ----> Saeb
      real*8,dimension(:,:),allocatable  ::       vv_old_implicit   !***** ----> Saeb

      real*8                                  :: metro_dzb_continuty           !***** ----> Saeb
      ...
      tolerance = 100.0d0
      tolerance_old = 500.0d0

      !impicit_iteration_threshold = 0    !3

      impicit_iteration_k = 0

      !water_depth_sea = 0.06
      !water_depth_river = 0.01
...
!********************************************************************************************************---->
      ...
      !do while ((abs((tolerance-tolerance_old)/tolerance) >= par%threshold_implicit_continuity) .or. (impicit_iteration_k <= impicit_iteration_threshold ))
      do while (impicit_iteration_k <= par%impicit_iteration_threshold)


         tolerance_old = tolerance
         zs_old_implicit = s%zs
         uu_old_implicit = s%uu
         vv_old_implicit = s%vv

         tolerance = 0.0d0


         ! if (par%water_level_sea_river_boundary_condition_activation == 1) then

            !if (par%num_boundary_layer_water_level >1) then

            !   s%zs(1:par%num_boundary_layer_water_level,1:s%ny+1) =  s%zb(1,1) + water_depth_sea             ! s%zs01                           !*****----> Saeb
            !   s%zs(s%nx-par%num_boundary_layer_water_level+1:s%nx+1,1:s%ny+1) =  s%zb(s%nx+1,1) + water_depth_river  ! s%zs03               !*****----> Saeb

            !else

          !  s%zs(1,1:s%ny+1) = s%zb(1,1:s%ny+1) + par%water_depth_sea                 ! s%zb(1,1) + water_depth_sea    !s%zs01                           !*****----> Saeb
          !  s%zs(s%nx+1,1:s%ny+1) = s%zb(s%nx+1,1:s%ny+1) + par%water_depth_river     ! s%zb(s%nx+1,1) + water_depth_river   ! s%zs03                      !*****----> Saeb

            !endif


            ! Save the values to a text file
            !open(unit=1, file='Xbeach_sea_river_water_level_source_code.txt', status='replace', action='write', iostat=iostatus)
            !open(unit=1, file='Xbeach_sea_river_water_level_source_code.txt', status='unknown',access='append', action='write', iostat=iostatus)
          !  open(unit=1, file='Xbeach_sea_river_water_level_source_code.txt', position='append', action='write', iostat=iostatus)

          !  write(1, *) par%t, s%zb(1, 1) + par%water_depth_sea, s%zb(s%nx+1,1) + par%water_depth_river

          !  close(unit=1)

        ! endif

         if (par%Dirichlet_water_level_activation==1) then

            call water_level_boundary(s,par)     !***** ----> Saeb
...
!********************************************************************************************************----> Saeb
                  if (par%momentum_dz_dx_activation == 1 ) then

                     s%uu(i,j) = s%uu(i,j) + ((par%metro_dzb/(0.5d0*par%metro_basinlength))*par%dt)
!********************************************************************************************************----> Saeb
                  endif
...
!********************************************************************************************************----> Saeb
         ...
         if (par%countinuty_with_Dzb_activation == 1) then
            ...
            if (s%ny>0) then
               ...
               s%zs(imin_zs:imax_zs,jmin_zs:jmax_zs) = s%zs(imin_zs:imax_zs,jmin_zs:jmax_zs) &
                  + (s%dzsdt(imin_zs:imax_zs,jmin_zs:jmax_zs)*par%dt) - (d_zb_dt(imin_zs:imax_zs,jmin_zs:jmax_zs)*par%dt)
         ...
         if (par%reduced_velocity_activation  == 1) then


            !s%uu(1:par%num_boundary_layer_water_level,1:s%ny+1) =par%reduced_velocity_parameter_boundary_sea_river* s%uu(1:par%num_boundary_layer_water_level,1:s%ny+1)                !*****----> Saeb
            !s%uu(s%nx-par%num_boundary_layer_water_level+1:s%nx+1,1:s%ny+1) = par%reduced_velocity_parameter_boundary_sea_river * s%uu(s%nx-par%num_boundary_layer_water_level+1:s%nx+1,1:s%ny+1)    !*****----> Saeb

            do i=1,par%num_boundary_layer_water_level
               do j=1,s%ny+1    !j=jmin_zs,jmax_zs

                  !WL_star_sea = s%zs(i,j)
                  !WL_star_river = s%zs(s%nx+1,j)

                  s%uu(i,j) = ((s%zs(i,j) - s%zb(i,j)) *  s%uu(i,j)) / (s%zs01 - s%zb(i,j) )

                  s%uu(s%nx-i+1:s%nx+1,j) = ((s%zs(s%nx-i+1:s%nx+1,j) - s%zb(s%nx-i+1:s%nx+1,j)) *  s%uu(s%nx-i+1:s%nx+1,j)) / (s%zs02 - s%zb(s%nx-i+1:s%nx+1,j) )
         ...
         !do j=1,s%ny+1
         !   do i=1,s%nx+1
         !s%uu(i,j) = ((relaxtaion_parameter)*s%uu(i,j)) + ((1-relaxtaion_parameter)*uu_old(i,j)) !***** Saeb
         !      s%uu(i,j) = ((par%relaxation_parameter)*s%uu(i,j)) + ((1-par%relaxation_parameter)*uu_previous_time_step(i,j)) !***** Saeb
         !      s%uu(i,j) = ((par%relaxation_parameter)*s%zs(i,j)) + ((1-par%relaxation_parameter)*zs_previous_time_step(i,j)) !***** Saeb
         !      s%uu(i,j) = ((par%relaxation_parameter)*s%vv(i,j)) + ((1-par%relaxation_parameter)*vv_previous_time_step(i,j)) !***** Saeb
...
!********************************************************************************************************----> Saeb
            ...
            do i = 1, s%nx+1
               do j = 1, s%ny+1
                  tolerance = tolerance + (zs_old_implicit(i, j) - s%zs(i, j))**2 + (uu_old_implicit(i, j) - s%uu(i, j))**2 + (vv_old_implicit(i, j) - s%vv(i, j))**2
               end do
            end do
      ...
      if (par%metro_period > 0.d0) then

         if (par%zb_updating_activation ==1) then

            s%zb=s%zb+(par%metro_dzb*(0.5d0*par%metro_basinlength-(s%xz-s%xz(1,1)))/(0.5d0*par%metro_basinlength))

         endif

         if (par%zs_updating_with_zb_activation ==1) then
            !s%zs(2:s%nx,:)=s%zs(2:s%nx,:)+par%metro_dzb*(0.5d0*par%metro_basinlength-(s%xz(2:s%nx,:)-s%xz(1,1)))/(0.5d0*par%metro_basinlength)
            s%zs=s%zs+par%metro_dzb*(0.5d0*par%metro_basinlength-(s%xz-s%xz(1,1)))/(0.5d0*par%metro_basinlength)
