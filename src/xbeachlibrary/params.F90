...
contains
   ...
   subroutine all_input(par)
      ...
      par%bchwiz      = readkey_int ('params.txt','bchwiz',        0,        0,     0,silent=.true.,strict=.true.)
      ...
      par%dtset   = readkey_dbl ('params.txt','dtset',   99.0d0,     0.001d0,   100.d0)                !--->**** Saeb
      ...
      par%wbctype==WBCTYPE_JONS_TABLE &
         ...
         par%random          = readkey_int ('params.txt','random',       0,          0,          1     ,strict=.true.)
      ...
      call setoldnames('0','1')
      if (par%ny==0) then
         call parmapply('left',2,par%left,par%left_str)
         call parmapply('right',2,par%right,par%right_str)
      else
      ...
      call parmapply('tidetype',3,par%tidetype,par%tidetype_str)
      ...
      call writelog('l','','--------------------------------')
      call writelog('l','','water level boundary conditions: ')

      par%water_level_loc_file          = readkey_name  ('params.txt','water_level_loc_file'                       )
      par%water_level_timeseries_file   = readkey_name  ('params.txt','water_level_timeseries_file'                )

      par%n_water_level              = get_file_length(par%water_level_loc_file                                 )
      par%nt_water_level             = get_file_length(par%water_level_timeseries_file                          )

      par%n_water_level              = readkey_int   ('params.txt', 'n_water_level',  par%n_water_level,  0, 100)
      par%nt_water_level             = readkey_int   ('params.txt', 'nt_water_level', par%nt_water_level, 0, 100)

      if (par%n_water_level>0) then
      
         call check_file_exist(par%water_level_loc_file)

         if (par%nt_water_level>0) then
            call check_file_exist(par%water_level_timeseries_file)
         endif
      endif
      ...
      if (par%swave==1) then
         ...
         elseif (par%wavemodel == WAVEMODEL_SURFBEAT) then
            call parmapply('break',4,par%break,par%break_str) ! default: roelvink2
      ...
      par%maxcf   = readkey_dbl ('params.txt','maxcf',     0.04d0,     0.0d0,   0.1d0)  ! max cf, only used for Manning and White Colebrook (Chezy: 15)
      ...
      if (par%sedtrans==1) then
         ...
         par%alfaD50    = readkey_dbl ('params.txt','alfaD50', 0.4d0, 0.0d0, 1.5d0, silent=.true.)
      endif
      !
      ...
      !
      ! Metronome simulation parameters
      par%metro_period = readkey_dbl ('params.txt','metro_period',   -1.d0,   0.d0,   3600.d0)
      

      if (par%metro_period>0.d0) then
         call writelog('l','','--------------------------------')
         call writelog('l','','Metronome simulation parameters: ')
         par%metro_omega       = 2.d0*par%px/par%metro_period
         par%metro_amp1        = readkey_dbl ('params.txt','metro_amp1',   0.05d0,    0.0d0,   0.1d0)
         par%metro_amp2        = readkey_dbl ('params.txt','metro_amp2',   0.01d0,    0.0d0,   0.1d0)
         par%metro_phase1      = readkey_dbl ('params.txt','metro_phase1',  0.0d0,    0.0d0,   360.d0)
         par%metro_phase2      = readkey_dbl ('params.txt','metro_phase2',  0.0d0,    0.0d0,   360.d0)
         par%metro_phase1      = par%metro_phase1 *par%px/180.d0
         par%metro_phase2      = par%metro_phase2 *par%px/180.d0
         par%metro_horzpart    = readkey_dbl ('params.txt','metro_horzpart', 5.0d0,    0.0d0,   10.d0)
         par%metro_basinlength = readkey_dbl ('params.txt','metro_basinlength',  20.0d0,    0.0d0,   100.d0)

         par%metro_period2      = readkey_dbl ('params.txt','metro_period2', -1.d0,   0.d0,   3600.d0)     !--->**** Saeb
         par%metro_omega2       = (2.d0*par%px*par%metro_period2)/par%metro_period                         !--->**** Saeb
         par%reduced_velocity_parameter_boundary_sea_river = readkey_dbl ('params.txt','reduced_velocity_parameter_boundary_sea_river', 1.d0,  0.d0,   1.d0)     !--->**** Saeb     
         par%num_boundary_layer_water_level = readkey_int  ('params.txt', 'num_boundary_layer_water_level', 1, 1, 100  ) !  Number of layers to specie water level boundary condition    ! ***** ----> Saeb
         par%relaxation_parameter = readkey_dbl ('params.txt','relaxation_parameter', 1.d0,  0.d0,   1.d0)     !--->**** Saeb     
        
         par%reduced_velocity_activation = readkey_int ('params.txt','reduced_velocity_activation',0,     0,         1,silent=.true.,strict=.true.)   ! ***** ----> Saeb
         par%zs_updating_with_zb_activation = readkey_int ('params.txt','zs_updating_with_zb_activation',0,     0,         1,silent=.true.,strict=.true.)   ! ***** ----> Saeb
         par%vy_zero_boundary_sea_river_activation = readkey_int ('params.txt','vy_zero_boundary_sea_river_activation',0,     0,         1,silent=.true.,strict=.true.)   ! ***** ----> Saeb
         par%countinuty_with_Dzb_activation = readkey_int ('params.txt','countinuty_with_Dzb_activation',0,     0,         1,silent=.true.,strict=.true.)   ! ***** ----> Saeb
         par%zero_gradient_u_activation = readkey_int ('params.txt','zero_gradient_u_activation',0,     0,         1,silent=.true.,strict=.true.)   ! ***** ----> Saeb
      ...
      if (par%front/=FRONT_NONH_1D) then
         if (par%wavemodel == WAVEMODEL_NONH) then
            call writelog('lws','','Warning: not using nonh_1d with wavemodel = nonh')
            !par%front = FRONT_NONH_1D Need to keep e.g. abs_1d for ship simulations
         endif
      endif
