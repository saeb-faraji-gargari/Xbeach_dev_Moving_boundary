module boundaryconditions
   ...
   save
   private
   public flow_lat_bc, wave_bc, flow_bc, discharge_boundary_h, discharge_boundary_v, water_level_boundary   !****-----> Saeb
contains
   subroutine wave_bc(sg,sl,par)
      ...
      ! write (*,*) "Dirichlet_water_level_activatio-------wave_bc: ", par%Dirichlet_water_level_activation

      !if (par%Dirichlet_water_level_activation==1) then
      !   write (*,*) "Dirichlet_water_level_activation is activate---------wave_bc"
      !   read(*,*) chert_pert                                 !****-----> Saeb
      !endif
   ...
   subroutine flow_bc(s,par)
      ...
      if (par%epsi<0.d0 .and. par%swave==1 .and. par%tidetype==TIDETYPE_HYBRID) then
         if(hybtavgtot<hybtavg) then
            ...
            if (xmpi_isbot) then
               hybmaxzs(2) = max(hybmaxzs(2),maxval(s%zs(s%ny+1,jmin_zs:jmax_zs)))
               hybminzs(2) = min(hybmaxzs(2),minval(s%zs(s%ny+1,jmin_zs:jmax_zs)))
               hybmaxzs0(2) = max(hybmaxzs0(2),maxval(s%zs0(s%ny+1,jmin_zs:jmax_zs)))
               hybminzs0(2) = min(hybmaxzs0(2),minval(s%zs0(s%ny+1,jmin_zs:jmax_zs)))
               hybmeanzs(2) = hybmeanzs(2)+par%dt/hybtavg* &
                  sum(s%zs(s%ny+1,jmin_zs:jmax_zs)*s%dnz(s%ny+1,jmin_zs:jmax_zs))/sum(s%dnz(s%ny+1,jmin_zs:jmax_zs))
               hybmeanzs0(2)= hybmeanzs0(2)+par%dt/hybtavg* &
                  sum(s%zs0(s%ny+1,jmin_zs:jmax_zs)*s%dnz(s%ny+1,jmin_zs:jmax_zs))/sum(s%dnz(s%ny+1,jmin_zs:jmax_zs))
            endif
      ...
      if (par%tidetype==TIDETYPE_INSTANT .or. par%tidetype==TIDETYPE_HYBRID) then
         ...
         do j=jmin_zs,jmax_zs
            do i=imin_zs,imax_zs
               if (s%zb(i,j)<s%zs0(i,j)) then  ! bed lower than zs0
      ...
      if (factime>0.d0 .and. (par%tidetype==TIDETYPE_VELOCITY .or. par%tidetype==TIDETYPE_HYBRID)) then
         if (xmpi_istop) then
            ...
            udnusum   = sum(s%uu (1,jmin_uu:jmax_uu)*s%dnu(1,jmin_uu:jmax_uu)*s%wetu(1,jmin_uu:jmax_uu))
            dnusum    = sum(s%dnu(1,jmin_uu:jmax_uu)*s%wetu(1,jmin_uu:jmax_uu))
            vdnvsum   = sum(s%vv (1,jmin_vv:jmax_vv)*s%dnv(1,jmin_vv:jmax_vv)*s%wetv(1,jmin_vv:jmax_vv))
            dnvsum    = sum(s%dnv(1,jmin_vv:jmax_vv)*s%wetv(1,jmin_vv:jmax_vv))
         else
            udnusum   = 0.d0
            dnusum    = 0.d0
            vdnvsum   = 0.d0
...
1000              continue
   ...
   ! water_level_boundary********************************************************************************************************----> Saeb


   subroutine water_level_boundary(s,par)
      use params,         only: parameters
      use spaceparamsdef
      use xmpi_module
      use interp,         only: linear_interp
      ...
      !real*8 :: chert_pert                        !****-----> Saeb
      !integer :: pause_activation

      !pause_activation = 0

      !if (par%Dirichlet_water_level_activation==1) then
      !write (*,*) "Dirichlet_water_level_activation is activate------water_level_boundary", par%Dirichlet_water_level_activation

      !s%zs(1,1:s%ny+1) = s%zb(1,1:s%ny+1) + par%water_depth_sea                 ! s%zb(1,1) + water_depth_sea    !s%zs01                           !*****----> Saeb
      !s%zs(s%nx+1,1:s%ny+1) = s%zb(s%nx+1,1:s%ny+1) + par%water_depth_river     ! s%zb(s%nx+1,1) + water_depth_river   ! s%zs03

      !write(*,*) "s%t_water_level(10): ", s%t_water_level(10)

      !write(*,*) "s%tdisch (10): ", s%tdisch(10)

      !write(*,*) "s%p_watre_level(1,1): ", s%p_watre_level(1,1)
      !write(*,*) "s%p_watre_level(1,2): ", s%p_watre_level(1,2)
      !write(*,*) "s%p_watre_level(1,3): ", s%p_watre_level(1,3)
      !write(*,*) "s%p_watre_level(1,4): ", s%p_watre_level(1,4)

      !s%qdisch(:,i)

      ! loop through discharge location
      !do  i = 1,par%n_water_level

      !do  i = 1,par%nt_water_level


      !write(*,*) "i: ", i
      !write(*,*) "s%t_water_level(i): ", s%t_water_level(i)
      !write(*,*) "s%q_water_level(i,1): ", s%q_water_level(i,1)
      !write(*,*) "s%q_water_level(i,2): ", s%q_water_level(i,2)
      ...
      do i = 1,par%n_water_level
        ...
        ! write(*,*) "s%p_watre_level(i,1): ", s%p_watre_level(i,1)
        ! write(*,*) "s%p_watre_level(i,2): ", s%p_watre_level(i,2)
        ! write(*,*) "s%p_watre_level(i,3): ", s%p_watre_level(i,3)
        ! write(*,*) "s%p_watre_level(i,4): ", s%p_watre_level(i,4)


         ! interpolate water level timeseries to current time
         call linear_interp(s%t_water_level,s%q_water_level(:,i),par%nt_water_level,par%t,qnow,indx)

        ! write(*,*) "qnow: ", qnow

         m1 = s%p_watre_level(i,1)    ! x index
         n1 = s%p_watre_level(i,2)    ! y index
         m2 = s%p_watre_level(i,3)    ! x index
         n2 = s%p_watre_level(i,4)    ! y index
         ...
         m1l = min(max(m1l,1),s%nx+1)
         m2l = min(max(m2l,1),s%nx+1)
         n1l = min(max(n1l,1),s%ny+1)
         n2l = min(max(n2l,1),s%ny+1)

        ! write(*,*) "befor ' if (n1.eq.n2) and if (m1.eq.m2)' sm1l: ", m1l,  "sm2l: ", m2l, "sn1l: ", n1l, "sn2l: ", n2l

         ! consider water level bundary condition in either horizontal or vertical line

         if (n1.eq.n2) then     ! Horizontal line
            ...
            if (indomain) then
              ...
              ! write(*,*) "after (n1.eq.n2) sm1l: ", m1l,  "sm2l: ", m2l, "sn1l: ", n1l, "sn2l: ", n2l
...
!#endif
            ...
            if (indomain) then
               ...
               if (n1l==s%ny+1) then
                  ...
                  s%zs(m1l:m2l,n1l-par%num_boundary_layer_water_level:n1l) = qnow

               elseif (n1l==1) then

                  s%zs(m1l:m2l,n1l:n1l+par%num_boundary_layer_water_level) = qnow

               else

                  s%zs(m1l:m2l,n1l-par%num_boundary_layer_water_level:n1l+par%num_boundary_layer_water_level) = qnow
         ...
         elseif (m1.eq.m2) then
            ...
            if (indomain) then
              ...
              ! write(*,*) "after (m1.eq.m2) sm1l: ", m1l,  "sm2l: ", m2l, "sn1l: ", n1l, "sn2l: ", n2l
...
!#endif
            ...
            if (indomain) then
               ...
               if (m1l==s%nx+1) then
                  ...
                  s%zs(m1l-par%num_boundary_layer_water_level:m1l,n1l:n2l) = qnow

               elseif (m1l==1) then

                  s%zs(m1l:m1l+par%num_boundary_layer_water_level,n1l:n2l) = qnow

               else

                  s%zs(m1l-par%num_boundary_layer_water_level:m1l+par%num_boundary_layer_water_level,n1l:n2l) = qnow
      ...
      enddo
   end subroutine water_level_boundary


   !End water_level_boundary********************************************************************************************************----> Saeb
