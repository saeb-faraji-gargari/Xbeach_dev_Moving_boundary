...
contains
   ...
   subroutine bed_update(s,par)
      ...
      integer                                     :: n_aval,im1,ip1,jm1,jp1, ntimesaval
      real*8,save                                 :: delta
      real*8                                      :: totslp,Sig
      ...
      if (par%t>=par%morstart .and. par%t < par%morstop .and. par%morfac > .999d0) then
         ...
         if (par%avalanching==1) then
            ...
            if (par%morfacopt==1) then
               ntimesaval=nint(par%morfac)
            else
               ntimesaval=1
            endif
            
            do ii=1,ntimesaval  !nint(par%morfac)
               !aval=.false.
               ...
               do i=2,s%nx !-1 Jaap -1 gives issues for bed updating at mpi boundaries
                  do j=1,s%ny+1
                     ...
                     if(max(hav(i,j),hav(i+1,j))>par%hswitch+par%eps) then ! Jaap instead of s%hh
                        dzmax=par%wetslp*abs(s%dzbdx(i,j))/totslp
                        ...
                        dzmax=par%dryslp*abs(s%dzbdx(i,j))/totslp
                     end if
               ...
               do j=2,s%ny !-1 Jaap -1 gives issues for bed updating at mpi boundaries
                  do i=1,s%nx+1
                     ...
                     if(max(s%hh(i,j),s%hh(i,j+1))>par%hswitch+par%eps) then
                        dzmax=par%wetslp*abs(s%dzbdy(i,j))/totslp
                     else
                        dzmax=par%dryslp*abs(s%dzbdy(i,j))/totslp
                     end if
