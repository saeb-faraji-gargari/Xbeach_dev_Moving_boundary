...
contains
   integer(c_int) function init()
      ...
      use xmpi_module
      use initialize_module,    only: drifter_init, wave_init, sed_init, flow_init, discharge_init, hotstart_init_1, hotstart_init_2, water_level_init  !*****-----> Saeb
      use initialize_module,    only: setbathy_init, grid_bathy
      use readtide_module,      only: readtide
      use readwind_module,      only: readwind
      use beachwizard_module,   only: bwinit
      ...
      call water_level_init   (s,par)       !*****-----> Saeb
   ...
   integer(c_int) function executestep(dt)
      ...
      if(xcompute) then
         ...
         if (error==0) then
            ...
            if ((par%flow==1).or.(par%wavemodel==WAVEMODEL_NONH).or.(par%Dirichlet_water_level_activation==1))   call flow_bc        (s,par)    !*****------> Saeb
            !
            ! Compute timestep
