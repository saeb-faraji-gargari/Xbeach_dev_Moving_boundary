module initialize_module
   ...
   implicit none
   private
   public setbathy_init, grid_bathy, drifter_init, wave_init, sed_init, flow_init, discharge_init, hotstart_init_1, hotstart_init_2, water_level_init   !*****-----> Saeb
   save
...
contains
   ...
   subroutine flow_init (s,par)
      ...
      s%dzsdy=0.d0
      s%dzbdt=0.d0
      !s%uu=0.d0  This wass killing the hotstart
      s%u=0.d0
      !s%vv=0.d0
      s%v=0.d0
      s%vu=0.d0
...
!********************************************************************************************************----> Saeb
   ...
   subroutine water_level_init(s, par)
      ...
      real*8, dimension(:),allocatable        :: xdb,ydb,xde,yde
      integer,dimension(2)                    :: mnb,mne
      logical                                :: io_exist_water_level_loc_file, io_exist_water_level_timeseries_file
      ...
      io          = 0
      io_exist_water_level_loc_file = .false.
      io_exist_water_level_timeseries_file = .false.

      allocate(xdb            (par%n_water_level)      )
      allocate(ydb            (par%n_water_level)      )
      allocate(xde            (par%n_water_level)      )
      allocate(yde            (par%n_water_level)      )

      !allocate(s%pnt_water_level     (1:par%n_water_level)                      )
      allocate(s%p_watre_level       (1:par%n_water_level   , 1:4)              )
      allocate(s%t_water_level       (1:par%nt_water_level)                     )
      allocate(s%q_water_level       (1:par%nt_water_level  , 1:par%n_water_level) )

      !s%pnt_water_level  = 0
      s%t_water_level    = 0.d0
      s%p_watre_level    = 0
      s%q_water_level    = 0.d0


      ! Check if the file exists
      inquire(file=par%water_level_loc_file, exist=io_exist_water_level_loc_file)
      inquire(file=par%water_level_timeseries_file, exist=io_exist_water_level_timeseries_file)

      if (.not. io_exist_water_level_loc_file) then

         write(*,*)  "Error: water_level_loc_file does not exist."

         stop

      endif

      if (.not. io_exist_water_level_timeseries_file) then

         write(*,*)  "Error: water_level_timeseries_file does not exist."
      ...
      if (par%n_water_level>0) then
         ...
         ! read discharge locations
         open(10,file=par%water_level_loc_file)
         do i=1,par%n_water_level
            read(10,*,IOSTAT=io) xdb(i),ydb(i),xde(i),yde(i)
            if (io .ne. 0) then
               call report_file_read_error(par%water_level_loc_file)
            endif
            ! distinguish between horizontal and vertical discharge
            !if (xdb(i).eq.xde(i) .and. ydb(i).eq.yde(i)) then
            !   s%pntdisch(i) = 1
         ...
         if (par%nt_water_level>0) then
            ...
            ! read time series
            open(10,file=par%water_level_timeseries_file)
            do i=1,par%nt_water_level
               read(10,*,IOSTAT=io) s%t_water_level(i),(s%q_water_level(i,j),j=1,par%n_water_level)
               if (io .ne. 0) then
                  call report_file_read_error(par%water_level_timeseries_file)
               endif
            enddo
            close(10)
         endif
      ...
      do i=1,par%n_water_level
         ...
         s%p_watre_level(i,:) = (/mnb(1),mnb(2),mne(1),mne(2)/)
         ...
         !s%p_watre_level(i,:) = (/mnb(1),mnb(2),0,0/)
         !          elseif (dxd.gt.dyd) then
         !elseif (mnb(1).ne.mne(1)) then
         ...
         !s%p_watre_level(i,:) = (/m1,n1,m1,n2/)
         !endif
      enddo

      ! incorporate morfac
      if (par%morfacopt == 1) then
         s%t_water_level = s%t_water_level/max(par%morfac,1.d0)
      endif
   end subroutine water_level_init
