   ...
   subroutine wave_stationary_directions(s,par,callType)
      ...
      do i=2,imax
         ...
         do while (stopiterate .eqv. .false.)
            ...
            gammafac_i = (par%gammax*hhwlocal(i,:))/s%H(i,:) ! inverse of gammafac = s%H(i,:)/(par%gammax*hhwlocal(i,:))
            do j=1,s%ny+1
            ...
            if(callType==callTypeStationary .and. par%rollergammax==1) then
               ...
               gammafac_i = (par%gammax*hhwlocal(i,:))/RH ! inverse of gammafac = s%H(i,:)/(par%gammax*hhwlocal(i,:))
               do j=1,s%ny+1
                  if (s%wete(i,j)==1 .and. gammafac_i(j)<1.d0) then
                     do itheta=1,ntheta_local
            ...
            if (iter<par%maxiter) then
               ...
               if(xmaster) then 
                  if (Herr>=par%maxerror) then
                     call writelog('lsw','(a,i4,a,i4,a,f5.4)','Wave propagation row ',i,', iteration ',iter,', H error: ',Herr)
                  endif
                  if (thetaerr>=par%maxerror_angle) then
                     call writelog('lsw','(a,i4,a,i4,a,f5.4)','Wave propagation row ',i,', iteration ', & 
                                    iter,', dir error: ',thetaerr/par%px*180)
                  endif
               endif
            endif
            !
